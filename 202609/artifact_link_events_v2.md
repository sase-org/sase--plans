---
tier: epic
title: Eliminate artifact-link merge conflicts with immutable link events (v2)
goal: "Automatic artifact-link writes never produce a merge conflict an agent must
  ...
  deterministically in Rust core and published through the host-owned machine lane,
  consuming the publication retry/aging ledger sase-yh landed instead of building a
  second one, while a conservative semantic resolver auto-repairs legacy links/*.json
...
phases:
  ...
  - id: event-contract
    ...
    description:
      ...
      sase-core with Python bindings, beside the landed publication-policy module."
  - id: outbox-identity
    ...
    description:
      ...
      stop count-collapsing distinct operations, and define retirement semantics that
      compose with the per-root publication retry ledger."
  - id: event-publisher
    ...
    description:
      ...
      them into the hidden host-owned clones through the existing commit choke point so
      the sase-yh retry ledger owns push retry."
  - id: event-readers
  ...
  - id: cutover-import
    ...
    description:
      ...
      baseline events, flag enablement and removal, read-only legacy indexes, doctor
      guardrails, and re-pointing the publication retry lane's legacy-index discovery at
      the event lane."
  - id: acceptance
    ...
    description:
      ...
      proving zero link-metadata merge pauses, no lost or double-counted operations,
      retry-ledger-driven recovery at every publication boundary, and clean crash
      recovery."
proposed_by: bbugyi200.athena.09d.f1
bead_id: sase-yy
create_time: 2026-09-09 19:52:14
status: wip
---

- **PROMPT:**
  [prompts/202609/artifact_link_events_v2.md](https://github.com/sase-org/sase--agents/blob/main/prompts/202609/artifact_link_events_v2.md)
- **BEAD:**
  [sase-yy](https://github.com/sase-org/sase--beads/blob/main/pages/sase-yy/README.md)

# Plan: Eliminate artifact-link merge conflicts with immutable link events (v2)

## Why this is a replan

Epic `bead:sase-yi` (plan `plan:202609/artifact_link_event_store.md`) proposed this same
migration and was canceled before any phase ran: it was chartered concurrently with epic
`bead:sase-yh`, claimed the same retry bug (`bead:sase-ye`), planned a second retry
ledger over the same push path, and its root phases collided with sase-yh's in-flight
edits. sase-yh has since landed completely and sase-ye, sase-yg, and sase-xi are closed
with implementation evidence. This plan carries the canceled epic's design forward — the
research recommendation is unchanged — re-chartered around what sase-yh actually landed.
Phase workers should treat THIS plan as authoritative where the two disagree; the
canceled plan remains linked for its fuller design rationale.

What sase-yh landed that changes the charter (all at current HEAD):

- **A per-root publication retry/aging ledger.** `commit_artifact_link_indexes`
  (`src/sase/sdd/_artifact_link_commit.py`) registers every failed push in
  `src/sase/sdd/_artifact_link_publication_retry.py`
  (`register_artifact_link_publication_failure`), keyed per project/role/repository by
  the Rust-side `artifact_link_publication_record_key`. The hourly
  `sase_chop_artifact_link_backfill` chop sweeps due records
  (`sweep_artifact_link_publication_retries`) and republishes unpublished heads.
  Backoff/aging policy is core-owned (`artifact_link_publication_register_pending`,
  `artifact_link_publication_due`, `artifact_link_publication_mark_attempt`, state wire
  v1) and the published floor is `sase-core-rs>=0.32.54`.
- **Non-destructive hidden-clone handling.** Missing upstream is a failure, not success;
  drift/dirty/unpublished hidden clones are preserved rather than replaced; role
  traversal is fair; probes and locks honor the chop deadline (commits a1b08d06c,
  1a32558a2, a5e642c92).
- **Publish eligibility / release evidence.** Outbox entries publish only when release
  evidence permits (`src/sase/sdd/artifact_link_release_evidence.py`, drain-side
  `_entry_is_eligible`) so one agent cannot release another run's queued rows (commit
  dd1f829c2).
- **`bead:sase-ye` is closed.** This epic makes no claim on it. The publisher phase
  consumes the ledger; it does not re-fix retry.
...
concurrent agents produce add/add and modify/modify git conflicts that an agent must
hand-resolve. Observed: agent `research.1n.cdx` (2026-09-08) hit a both-added conflict
on a research-sidecar index, repaired it by hand, and hit a second conflict in the same
file on `sase stitch create --resume`. Roughly 28% of research-sidecar commits are
link-only, so the contention window is constantly refreshed. The consolidated research
report `research:202609/artifact_link_event_store/artifact_link_event_store.md` analyzes
the failure and recommends the architecture this epic implements. Read it (audited, via
...
2. `upsert_row` (`src/sase/sdd/_artifact_link_store_rows.py`) writes each document
   ...
   (`src/sase/sdd/_artifact_link_store_sidecar.py`), a whole-file read-modify-replace
   under a per-clone flock that cannot help across clones.
...
sase-yh fixed what happens after a failed push; it did not change what conflicts. The
two failure modes remain: sidecar publication aborts and retries against a moving remote
(now durably, via the ledger — but each retry can hit the same conflict), and the
primary-commit path pauses and burns an agent turn on hand-repair.

## Current architecture facts phase workers need

- **Row model (schema v2).** Rust owns it: `crates/sase_core/src/artifact_link/` in the
  linked `sase-core` repo (`ArtifactLinkRowWire`, `ArtifactLinkIndexWire`,
...
- **Storage split.** Document-kind endpoints go to sidecar `links/*.json`; `bead:`
  ...
  rebuildable machine-local aggregate. `NON_SIDECAR_KINDS` and path helpers live in
  `src/sase/sdd/_artifact_link_store_support.py`.
- **Machine write lane (epic sase-y3, hardened by sase-yh).** Background link
  maintenance never writes into a primary checkout's nested sidecar clones:
  `src/sase/sdd/_artifact_link_machine_store.py` resolves hidden host-owned clones
  integrated from the remote per use; every background writer must pass
  `authorize_store_mutation(root, mutation_origin="machine")`
  (`src/sase/sdd/_artifact_link_authorize.py`). Audited reads still write the agent's
  own workspace sidecar clone — that is the lane this epic closes.
- **Commit choke point and retry.** `commit_artifact_link_indexes`
  (`src/sase/sdd/_artifact_link_commit.py`) stages only paths passing the canonical
  checks in `src/sase/sdd/_artifact_link_files.py`, pushes through
  `run_managed_sync_worker` (`src/sase/bead/sync_worker.py`), and on push failure
  registers the unpublished head with the per-root retry ledger described above. Any
  writer that commits through this choke point inherits durable retry for free.
- **Outbox at HEAD.** `src/sase/sdd/artifact_link_outbox.py` keeps a machine-local JSONL
  queue (schema v1, short uuid ids, `agent_name`/`run_id` provenance).
  `drain_artifact_link_outbox` partitions entries (agent selection, stale-terminal drop
  lane with audit trail, release-evidence eligibility, machine writability), replays
  rows via `_converged_rows` — which still collapses same-logical-key entries, losing
  distinct repeated reads as separate operations — commits through the choke point with
  `push_after_commit="async"`, and retires drained ids after commit
  (`_rewrite_without_ids`). Push retry of a committed-but-unpublished head is the
  ledger's job, not the outbox's.
...
4. **Publication rides the landed retry lane; no second ledger.** Automatic writers
   durably enqueue first and update the local pending view; one publisher per
   project/machine batches operations into the hidden host-owned clones and commits
   through `commit_artifact_link_indexes`, so a failed push registers in the sase-yh
   per-root ledger and the hourly sweep owns republication with backoff and aging. The
   outbox journal owns per-operation retirement (an operation is retired only once its
   event object is durably committed in the owning hidden clone); the ledger owns
   per-root push retry of committed heads. These two layers must compose without
   overlap: the publisher itself performs no unbounded retry loop of its own.
...
the binding floor from its current `>=0.32.54` the way sase-yh did and run both repos'
verification (`just check` here; the sase-core repo's own gate there).
...
the observed failure class. Wiring points verified at current HEAD.
...
- **Python.** Add a links conflict resolver module beside
  ...
  (stage reads, rebase-aware ours/theirs ordering, `git_add`) and writing the exact
  bytes the normal writer produces. Handle:
...
- **Wire both paths.**
  ...
  - `_continue_rebase_resolving_beads` (`_git_commit_dispatch.py`): generalize
    ...
    agent-facing paused-conflict message for unclaimed paths. This file also carries
    sase-yh's checkpoint-recovery changes; extend, do not regress, that behavior.
...
All in sase-core, additive, inert until later phases call it. The new event module lands
beside the existing `artifact_link` wire/publication-policy code; do not disturb the
landed publication policy.
...
- **Bindings:** expose validate/canonicalize/reduce/merge-path helpers through
  ...
  list, and ratchet the Python-side binding floor from `>=0.32.54`.
...
- Delete the count-collapsing behavior of `_converged_rows` for v2 entries: distinct
  ...
  operations. Preserve the v1 convergence (including its existing-plus-increment uses
  arithmetic) only for leftover v1 entries.
- Preserve the drain lanes sase-yh-era work added: agent selection, the stale-terminal
  drop lane and its audit trail, release-evidence eligibility (`_entry_is_eligible`),
  and machine-writability partitioning all still apply to v2 entries.
- Retirement composes with the retry ledger: a v2 entry leaves the queue only after its
  event object is durably committed in the owning hidden host-owned clone.
  Push-after-commit failures are the per-root ledger's job — do not hold entries hostage
  to remote publication, and do not ack before the commit exists. Survive interruption
  after file creation, after commit, and before acknowledgement — idempotent replay by
  content address makes re-publication safe.
- Local pending view: queued-but-unpublished operations must be visible to the aggregate
  rebuild (event-readers consumes this; this phase provides an API to enumerate pending
  events).
...
- **Publisher.** A per-project/machine publisher (mirror `run_managed_sync_worker`
  ...
  `resolve_machine_artifact_link_store`, one commit per batch per repo through
  `commit_artifact_link_indexes`, then acknowledges retirement per rule 4. Extend
  `src/sase/sdd/_artifact_link_files.py` so canonical `link-events/v1/**` paths are
  committable and `_artifact_link_commit.py` stages them; confirm the retry ledger's
  registration and sweep cover event-object commits exactly as they cover index commits
  (extend `sweep_artifact_link_publication_retries` coverage tests to an event-lane
  root). Ensure publish-eligibility (release evidence) treats event-only batches as real
  publishable changes. Publication failures leave a ledger-registered retryable head —
  never an agent merge task and never a publisher-local retry loop.
...
- **Tests:** flag-off parity (existing suites pass unchanged), flag-on enqueue-only
  behavior, publisher batching, ledger-registration on push failure plus sweep-driven
  republication, crash-injection between create/commit/ack, and the hidden-clone e2e
  pattern from `tests/sdd/test_artifact_link_hidden_clone_e2e.py`.
...
- **Doctor** (`src/sase/artifact_cli/link_health.py`, `doctor.py`): event-store health
  ...
  age surfaced, unpublished batches, and per-root ledger aging surfaced alongside) joins
  the existing checks.
...
- **Re-point the retry lane.** sase-yh's ledger and sweep were built while legacy index
  commits were the publishable unit. After the fence: event-lane commits must remain
  fully covered (registration, backoff, aging report); any discovery or backfill step
  that exists only to find unpublished legacy-index work is retired or narrowed with
  tests proving no event-lane regression; the backfill chop's remaining legacy
  responsibilities are re-examined and documented. Do not delete ledger state for roots
  that still have unpublished legacy heads.
...
- **Tests:** import determinism (same input heads -> byte-identical baseline events),
  ...
  it, resolver still merges it, nothing is silently lost), event-lane retry coverage
  after legacy discovery retires.
...
- crash/kill at every publication boundary (after event write, after commit, after push,
  before acknowledgement) followed by recovery — including at least one scenario
  recovered purely by the per-root ledger sweep republishing a committed head, with no
  agent involvement and no duplicate operations;
